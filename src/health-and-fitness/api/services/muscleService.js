...
const getAllMuscles = async () => {
  try {
    const musclesSnapshot = await getDocs(collection(firestoreDb, "muscles"));
    if (musclesSnapshot.empty) {
      return { error: "No muscles found", status: 404 };
    }

    const muscles = musclesSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));

    return { muscles, status: 200 };
  } catch (error) {
    console.error("Error getting all muscles:", error);
    return { error: error.message, status: 500 };
  }
};
