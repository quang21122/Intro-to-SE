...
const MyPlansEdit: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchExerciseDetails = async (exerciseId: string) => {
      try {
        const response = await fetch(
          `https://intro-to-se-pi.vercel.app/api/exercise?id=${exerciseId}`
        );
        if (!response.ok) {
    ...
    const fetchPlan = async () => {
      if (user) {
        ...
        try {
          const response = await fetch(
            `https://intro-to-se-pi.vercel.app/api/myPlan?uid=${user.uid}`
          );
          ...
          const plansWithDetails: Plan[] = await Promise.all(
            plans.map(async (plan: Plan) => {
              const detailsResponse: Response = await fetch(
                `https://intro-to-se-pi.vercel.app/api/myPlan?uid=${user.uid}&id=${plan.id}`
              );
  ...
  const handleAddExercise = (exercise: Exercise, selectedDay: number) => {
    ...
    fetch(`https://intro-to-se-pi.vercel.app/api/myPlan?uid=${user?.uid}&id=${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
