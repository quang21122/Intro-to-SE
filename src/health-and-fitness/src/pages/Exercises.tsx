    ...
    const fetchExercises = async () => {
      try {
        ...
        const exerciseRes = await fetch(
          debouncedSearchTerm
            ? `https://intro-to-se-server.vercel.app/api/exercise?search=${debouncedSearchTerm}` // Fetch exercises with search term
            : selectedMuscles.length || selectedEquipment.length
            ? `https://intro-to-se-server.vercel.app/api/exercise?${queryParam.toString()}` // Fetch exercises with selected muscles
            : `https://intro-to-se-server.vercel.app/api/exercise?page=${currentPage}`
        );
